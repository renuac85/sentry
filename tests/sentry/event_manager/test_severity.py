...
class TestGetEventSeverity(TestCase):
    ...
    @pytest.mark.xfail(
        reason="TODO (kmclb): Failing because of changes made in XXXX, and because the test is too brittle and the behavior it's testing is based on flawed assumptions (says the person who wrote the code in the first place). Will be fixed in a follow-up PR."
    )
    @patch(
        "sentry.event_manager.severity_connection_pool.urlopen",
        return_value=HTTPResponse(body=json.dumps({"severity": 0.1231})),
